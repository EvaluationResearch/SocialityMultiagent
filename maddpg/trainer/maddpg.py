...
def p_train3(make_obs_ph_n, act_space_n, p_index, p_func, q_func, optimizer, adversarial, adv_eps, adv_eps_s,
...
# p_train建立actor 建立 critic 然后将critic 的输出 和 actor输出 当做loss 来训练actor
...
def q_train3(make_obs_ph_n, act_space_n, q_index, q_func, optimizer, adversarial, adv_eps, adv_eps_s, num_adversaries, grad_norm_clipping=None, local_q_func=False, scope="trainer", reuse=None, num_units=64):
    with tf.variable_scope(scope, reuse=reuse):
        # create distribtuions
        act_pdtype_n = [make_pdtype(act_space) for act_space in act_space_n]
...
#critic 建立 critic 然后建立 traget critic
def q_train(make_obs_ph_n, act_space_n, q_index, q_func, optimizer, grad_norm_clipping=None, local_q_func=False, scope="trainer", reuse=None, num_units=64):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        return train, update_target_q, {'q_values': q_values, 'target_q_values': target_q_values}
class MADDPG3AgentTrainer(AgentTrainer):
    def __init__(self, name, model, obs_shape_n, act_space_n, agent_index, args, local_q_func, policy_name, adversarial):
        ...
        self.q_train, self.q_update, self.q_debug = q_train3(
            scope=self.scope,
        ...
        self.act, self.p_train, self.p_update, self.p_debug = p_train3(
            scope=self.scope,
            make_obs_ph_n=obs_ph_n,
            act_space_n=act_space_n,
            p_index=agent_index,
        ...
        # change experience buffer size
        # self.replay_buffer = ReplayBuffer(1e6)
        self.replay_buffer = ReplayBuffer(1e8)
...
class MADDPGAgentTrainer(AgentTrainer):
    def __init__(self, name, model, obs_shape_n, act_space_n, agent_index, args, local_q_func=False):
        ...
        # Create all the functions necessary to train the model(critic)
