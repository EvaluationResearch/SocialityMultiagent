...
class SoftCategoricalPd(Pd):
    ...
    def entropy(self):
        ...
        return U.sum(p0 * (tf.log(z0) - a0), axis=1)
    def sample(self):
        u = tf.random_uniform(tf.shape(self.logits)) #Output random value from uniform distribution
        return U.softmax(self.logits - tf.log(-tf.log(u)), axis=-1)  #Note: Gumbel softmax was mentioned in the original paper
    @classmethod
    def fromflat(cls, flat):
        return cls(flat)        
